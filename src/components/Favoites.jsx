...
import propertyImage from './assets/homes.jpg';
...
const FavoriteProperties = () => {
  ...
  const mapApiDataToCategories = (apiData) => {
    ...
    return apiData.data.map((item, index) => ({
      ...
      price: item.Properties?.Property_Listing_Price
        ...
        : staticCategories[index % staticCategories.length].price,
      address: `${item.Properties?.Property_Address || ''}, ${item.Properties?.Property_city || ''}, ${
        item.Properties?.Property_state || ''
      }, ${item.Properties?.Property_country || ''}`.trim() || staticCategories[index % staticCategories.length].address,
      beds: item.Properties?.Property_Bed_rooms || staticCategories[index % staticCategories.length].beds,
      baths:
        (item.Properties?.Property_Full_Baths || 0) + (item.Properties?.Property_OneTwo_Baths || 0) ||
        staticCategories[index % staticCategories.length].baths,
      size: item.Properties?.Property_finished_Sq_ft || staticCategories[index % staticCategories.length].size,
      status: item.Properties?.Properties_for || staticCategories[index % staticCategories.length].status,
      promoted: staticCategories[index % staticCategories.length].promoted,
      featured: staticCategories[index % staticCategories.length].featured,
      image:
        item.Properties?.Property_photos?.[0]?.image
          ? `${process.env.REACT_APP_BASE_URL}/images/${item.Properties.Property_photos[0].image}`
          : staticCategories[index % staticCategories.length].image,
    }));
  };
  ...
  const categories = isLoading
    ...
    : isError
    ? staticCategories
    : mapApiDataToCategories(favoritePropertiesData);
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mx-12 px-6">
        <div className="md:w-1/2 mb-8 md:mb-0">
          <h2
            ...
            className="text-2xl md:text-[46px] font-bold text-gray-800 md:mt-0 mt-6 leading-snug"
          >
      ...
      >
        ...
        {isError && (
          <div className="p-4 text-center text-red-600">
            <p>Error loading Favorite Properties: {error?.data?.message || 'An error occurred'}</p>
