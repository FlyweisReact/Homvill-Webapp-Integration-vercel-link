...
const AccordionItem = ({ item, index, openIndex, setOpenIndex }) => {
  ...
  return (
    <div style={{ fontFamily: 'Poppins' }} className="border-b border-gray-200 py-4">
      ...
      >
        <span
          className={`font-semibold text-sm md:text-[18px] ${isOpen ? 'text-[#8A1538]' : 'text-gray-900'
            }`}
        >
          {item.question}
        </span>
...
const FaqSection = () => {
  ...
  const [openRight, setOpenRight] = useState(2);
  const isAuthenticated = useSelector(selectIsAuthenticated);
  const { data: faqsData, isLoading, isError, error } = useGetAllFAQsQuery();

  // Static fallback data
  const staticFaqsLeft = [
    {
    ...
    if (isError) {
      return (
        <div className="text-center text-red-600">
          <p>Error loading FAQs: {error?.data?.message || 'An error occurred'}</p>
          <p>Displaying fallback data.</p>
        </div>
      );
    }
    const faqs = faqsData?.success && faqsData?.data?.length
      ? faqsData.data.map(item => ({
        question: item.Title,
        answer: item.Description,
      }))
      : [...staticFaqsLeft, ...staticFaqsRight];

    // Split FAQs evenly between left and right
